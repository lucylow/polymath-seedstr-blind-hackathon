...
import { Brain, Search, Code, Paintbrush, Gauge, Terminal, Bot, List, Eye, FileCode, Download, Loader2, CheckCircle2, Clock, ArrowLeft } from "lucide-react";
import { Button } from "@/components/ui/button";

const AGENTS = [
  { id: "supervisor", name: "Supervisor", icon: Brain, colorClass: "bg-secondary" },
  { id: "analyst", name: "Analyst", icon: Search, colorClass: "bg-primary" },
  { id: "developer", name: "Developer", icon: Code, colorClass: "bg-accent" },
  { id: "designer", name: "Designer", icon: Paintbrush, colorClass: "bg-yellow-500" },
  { id: "optimizer", name: "Optimizer", icon: Gauge, colorClass: "bg-destructive" },
] as const;
...
const runAgentWorkflow = async (
  ...
  set("supervisor", "running", "Initializing…");
  await sleep(800);
  set("supervisor", "done", "Ready");
  onLog("Supervisor ready");

  set("analyst", "running", "Analyzing prompt…");
  await sleep(1500);
  set("analyst", "done", "Plan created");
  onLog("Analyst created technical plan");
  ...
  set("developer", "done", "Code ready");
  onLog("Developer finished writing code");
  set("designer", "done", "Styles ready");
  onLog("Designer finished creating styles");

  set("optimizer", "running", "Optimizing…");
  await sleep(1200);
  set("optimizer", "done", "Optimization complete");
  onLog("Optimizer finished – minified and improved");

  set("supervisor", "running", "Packaging…");
  await sleep(500);
  set("supervisor", "done", "Submission ready");
  onLog("✅ All done! Output ready for download.");
...
export default function Dashboard() {
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = "polymath-output.txt";
    a.click();
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-[1400px] mx-auto p-4 md:p-8">
        ...
        <div className="flex items-center gap-4 mb-2">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold gradient-text">Full‑Stack Polymath</h1>
        </div>
        <p className="text-muted-foreground text-lg mb-8 ml-14">Multi‑agent AI system that builds anything from a mystery prompt</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="glass-card rounded-2xl p-6">
              ...
              <textarea
                className="w-full p-4 bg-muted border border-border rounded-xl text-foreground placeholder:text-muted-foreground resize-y focus:outline-none focus:ring-2 focus:ring-ring mb-4"
                rows={3}
            ...
            <div className="glass-card rounded-2xl p-6">
              ...
              <div ref={logRef} className="bg-muted rounded-xl p-4 font-mono text-sm max-h-[250px] overflow-y-auto space-y-1">
                ...
                {logs.map((l, i) => (
                  <div key={i} className="text-muted-foreground border-l-2 border-border pl-2">
          ...
          <div>
            <div className="glass-card rounded-2xl p-6">
              ...
              <div className="bg-white rounded-2xl overflow-hidden h-[400px] resize-y">
                {result ? (
              ...
              {result && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground mt-2">* In the real system, this would be a proper .zip file.</p>
                </>
              )}
            </div>
          </div>
